...
struct DashboardView: View {
    ...
    private var metrics: some View {
        ...
        return LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 14), count: 3), spacing: 14) {
            ...
            MetricCard(title: lang.select("Cache Creation", "缓存创建"), value: formatTokens(snapshot.selected.cacheCreationTokens), subtitle: lang.select("Cache read \(formatTokens(snapshot.selected.cacheReadTokens))", "缓存读取 \(formatTokens(snapshot.selected.cacheReadTokens))"), accent: .neonBlue)
